...
extension XCUIScreenshot {
    func getBase64EncodedString() -> String? {
        return self.image.jpegData(compressionQuality: 0.1)?.base64EncodedString()
    }
}
    ...
    static let modelName: String = {
        ...
        func mapToDevice(identifier: String) -> String { // swiftlint:disable:this cyclomatic_complexity
            ...
            case "i386", "x86_64", "arm64":
                let simulatorIdentifier = mapToDevice(identifier: ProcessInfo().environment["SIMULATOR_MODEL_IDENTIFIER"] ?? "iOS")
                return identifierToDevice[simulatorIdentifier] ?? simulatorIdentifier
            default:
                return identifierToDevice[identifier] ?? identifier
            }
#elseif os(tvOS)
            switch identifier {
            case "i386", "x86_64":
                let simulatorIdentifier = mapToDevice(identifier: ProcessInfo().environment["SIMULATOR_MODEL_IDENTIFIER"] ?? "tvOS")
                return identifierToDevice[simulatorIdentifier] ?? simulatorIdentifier
            default:
                return identifierToDevice[identifier] ?? identifier
            }
...
let identifierToDevice: [String: String] = [
