...
<CardGroup cols={2}>
  <Card title="FinalRun Cloud — recommended" icon="cloud" href="/configuration/cloud-api-key">
    Sign up at [cloud.finalrun.app](https://cloud.finalrun.app) and set `FINALRUN_API_KEY`. Managed AI, no provider account needed, and new accounts get $5 of free credits. Setup steps: [Cloud API Key](/configuration/cloud-api-key).
  </Card>
  <Card title="Bring your own provider key" icon="key" href="/configuration/ai-providers">
    Use your own Google, OpenAI, or Anthropic API key (`GOOGLE_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`). Standard provider billing applies. Setup steps: [AI Providers](/configuration/ai-providers).
  </Card>
</CardGroup>
