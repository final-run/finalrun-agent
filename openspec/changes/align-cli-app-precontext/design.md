...
1. Add the new primary-app context types and runtime plumbing behind additive optional fields.
2. Resolve and pre-launch the primary app during CLI setup only when the run has explicit primary-app input.
3. Wire planner `preContext` and `appKnowledge` from the run/session layer into goal execution.
4. Update launch defaults for known primary-app relaunches.
5. Extend tests and run artifact snapshots to cover the new context.

Rollback is straightforward because the change is additive: remove primary-app context resolution and stop passing planner pre-context, which returns the CLI to the current "install only, no pre-launch memory" behavior.

## Open Questions

- What is the preferred Android package-name resolution strategy for `--app` in this repo: local APK manifest parsing, host-tool-based extraction, or device-side discovery after install?
- Should repo-level app knowledge authoring be included in this change, or should this change only wire the optional field and defer the authoring source?
